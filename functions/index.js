...
cron.schedule("0 */8 * * *", () => {
  reporte.get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
      const data = doc.data();
      const hasServicio = Object.prototype.hasOwnProperty.call(
        data,
        "servicio"
      );
      const hasConfirmacion = Object.prototype.hasOwnProperty.call(
        data,
        "confirmacion"
      );

      if (hasServicio && !hasConfirmacion)
        reminderConfirmationEmail(data.id, data.email_contacto);
    });
  });
});
...
app.post("/recuperar", async (req, res) => {
  const { email, nuevaPassword, token } = req.body;
  ...
  if (!token && !nuevaPassword) {
    ...
    const link = `http://localhost:4200/recuperar?email=${email}&token=${nuevoToken}`;
  ...
  if (token && nuevaPassword) {
    if (usuario.token !== token || usuario.expiracion < Date.now()) {
